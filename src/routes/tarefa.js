...
router.put('/editar/:id', authUser, conectarBancoDados, async function (req, res) {
    try {
        ...
        if(!checkTarefa){
            throw new Error("Tarefa não encontrada ou poertence a outro usuário");
        } 
        ...
        if(tarefaAtualizada?.modifiedCount > 0){
            ...
            res.status(200).json({
                status: "OK",
                statusMensagem: "Tarefa editada com sucesso.",
...
router.get('/obter/usuario', authUser, conectarBancoDados, async function (req, res) {
    try {
        ...
        res.status(200).json({
            status: "OK",
            statusMensagem: "Tarefas obtidas e listadas com sucesso.",
            resposta: respostaBD
        })
